...
int main() {
    int N = 0, i = 0, j = 0, prio = 0, prio_mid = 0, found = 0, prio2 = 1, prio3 = 1;
    scanf("%d", &N);
    int deplasare = N-2;
    ...
    for (j = 1; deplasare >= 0; j++) {
        found = 0;
        if ((op[j] == '+' || op[j] == '-') && prio == 0 && prio_mid == 0) {
            if (op[j] == '+') {
                res = term[j-1] + term[j];
            }
            if (op[j] == '-') {
        ...
        if (op[j] == '#' && prio == 0) {
            res = (term[j-1] + term[j])*(term[j-1] + term[j]);
            prio_mid--;
            found = 1;
        }
